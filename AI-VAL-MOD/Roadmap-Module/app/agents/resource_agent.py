    ...
    def run(self, tasks: List[Dict], team_members: List[Dict], business_type: str) -> List[Dict]:
        ...
        prompt = RESOURCE_PROMPT.format(
            ...
            tasks_json=json.dumps(
                [{"task_id": t["task_id"], "title": t["title"], "description": t.get("description", "")} for t in tasks],
                indent=2,
            ),
        )
        ...
        try:
            ...
            print(f"[{self.name}] JSON parse failed — returning tasks unassigned")
            return tasks

        enrich_map = {e["task_id"]: e for e in enrichments}
        for task in tasks:
